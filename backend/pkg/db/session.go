...
type SessionWriter interface {
	WriteASession(speakerID *int, roomID *int, timeslotID *int, name *string) (int64, error)
}

// SessionUpdater implements all update related methods
type SessionUpdater interface {
	UpdateASession(sessionID int64, speakerID *int, roomID *int, timeslotID *int, name *string) error
}
