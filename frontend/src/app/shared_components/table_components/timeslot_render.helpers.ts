...
export class TimeslotRenderHelpers {
  static formatTimeForCell(
    time: string,
    timeslotGlobals: TimeslotGlobals
  ): string {
    if (time.includes(" ")) {
      return TimeslotRenderHelpers.formatTimeForCellStartDashEnd(
        time,
        timeslotGlobals
      );
    } else {
  ...
  static formatTimeForCellStartDashEnd(
    time: string,
    timeslotGlobals: TimeslotGlobals
  ): string {
    ...
    if (
      TimeslotGlobals.isValidTime(startTime) &&
      TimeslotGlobals.isValidTime(endTime)
    ) {
      return `${TimeslotRenderHelpers.formatTimeForCellSingleTime(
        startTime,
        timeslotGlobals
      )}-${TimeslotRenderHelpers.formatTimeForCellSingleTime(
        endTime,
        timeslotGlobals
      )}`;
    } else {
  ...
  static formatTimeForCellSingleTime(
    time: string,
    timeslotGlobals: TimeslotGlobals
  ): string {
